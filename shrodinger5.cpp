...
int main (void)
{
    ...
    //Otras variables
    complex<double> phi[N2+1], phi0[N2+1], xi[N2+1], aux3;
    complex<double> A0[N2], b[N2], beta[N2], alpha[N2], gamma[N2];
    double modulo;
    double Aplus, Aminus;
    double k0virg, V[N2+1], svirg;
    ...
    {   
        ...
        else if ((aux1>=4*aux2)&&(aux1<=5*aux2)) V[j]=lambda*k0virg*k0virg;
        else if ((aux1>=6*aux2)&&(aux1<=7*aux2)) V[j]=lambda*k0virg*k0virg;
        else if ((aux1>=8*aux2)&&(aux1<=9*aux2)) V[j]=lambda*k0virg*k0virg;
        else if ((aux1>=10*aux2)&&(aux1<=11*aux2)) V[j]=lambda*k0virg*k0virg;
        else if ((aux1>=12*aux2)&&(aux1<=13*aux2)) V[j]=lambda*k0virg*k0virg;
        else V[j]=0.;
    }
    ...
    modulo=0.;
    for (j=1; j<N2; j++) modulo= modulo + real(phi0[j])*real(phi0[j]) + imag(phi0[j])*imag(phi0[j]);
    modulo=sqrt(modulo);
    ...
    Aplus=1.;
    Aminus=1.;
    for (j=1; j<N2; j++) A0[j]=complex<double>(-2.-V[j],2*Aminus/svirg);
    beta[N2-1]=0.;
    alpha[N2-1]=0.;
    gamma[N2-1]=Aminus/A0[N2-1];
    for (j=N2-2; j>=0; j--) 
    {
    ...
    for (l=0; l<m; l++)
    {
        ...
        do{
            ...
            for (n=0; n<=nd; n++)
            {
                for (j=1; j<N2; j++) b[j]=complex<double>(0, 4)*phi[j]/svirg;
                for (j=N2-2; j>=0; j--) beta[j]=gamma[j+1]*(b[j+1]-beta[j+1]);    
            ...
            transmision=0.;
            for (j=N2-aux2; j<=N2; j++) transmision = transmision + real(phi[j])*real(phi[j]) + imag(phi[j])*imag(phi[j]);
            ...
            {
                for (j=N2-aux2; j<N2; j++) phi[j]=0.;
                modulo=0.;
                for (j=1; j<N2; j++) modulo= modulo + real(phi[j])*real(phi[j]) + imag(phi[j])*imag(phi[j]);
                modulo=sqrt(modulo);
                for (j=1; j<N2; j++) phi[j]=phi[j]/modulo;
